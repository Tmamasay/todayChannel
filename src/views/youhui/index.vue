    ...
    <div class="fenlei_admin">
      ...
      <div class="add_fenlei">
        ...
        <el-dialog
          ...
          style="width:85%;margin:0 auto"
        >
          <el-form v-if="Guizevisible" ref="guizeForm" :rules="addGuizerules" :model="guizeForm" label-width="110px">
            <el-form-item label="积分价格：" prop="integral">
              <el-input v-model="guizeForm.integral" placeholder="请输入积分价格" />
            </el-form-item>
            <el-form-item label="库存数量：" prop="stock">
              <el-input v-model="guizeForm.stock" placeholder="请输入库存数量" />
            </el-form-item>
          </el-form>
          <div class="el-center" style="margin-left:37%;margin-top:30px;">
            <el-button type="primary" @click="guizSubmit('guizeForm')">提交</el-button>
            <el-button plain @click="Guizevisible=false">取消</el-button>
          </div>
        </el-dialog>
      </div>
    </div>
...
export default {
  ...
  methods: {
    guizSubmit() {
      setVoucherStock({
        id: this.guizeForm.id,
        integral: this.guizeForm.integral,
        stock: this.guizeForm.stock
      }).then(res => {
        if (res.status) {
          this.$message({ message: '操作成功', type: 'success' })
          this.Guizevisible = false
          this.getlist()
        }
      })
    },
    getRule(row) {
      ...
      this.guizeForm.integral = row.integral || 0
      this.guizeForm.stock = row.stock || 0
      this.guizeForm.id = row.id
    },
    downupYh(row) {
